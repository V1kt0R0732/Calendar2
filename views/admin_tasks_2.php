...
<div class="container">
                ...
                <div class="task-content">
                    ...
                    <form class="task-actions" action="/manage/add" method="post">
                        ...
                        <select id="assign" class="assign-select" name="worker">
                            <?php $temp = true; foreach($workers as $worker):
                                if(!empty($task['employees_arr'])){
                                    ...
                                    <?php } } else { $temp = false; ?>
                                    <option value="<?=$worker['id']?>"><?=$worker['name']?></option>
                                <?php } endforeach; if($temp){ ?>
                                <option value="">No People</option>
                            <?php } ?>
                        </select>
                        <?php if(isset($task['id']) && !empty($task['id'])){ ?>
    ...
    <div style="display: flex;justify-content: space-around;">
        <a href="/show/project/complete/<?=$id?>/1" class="view-tasks-link">Выполненые Задачи</a>
        <a href="/all/project" class="view-tasks-link">Назад к проектам</a>
