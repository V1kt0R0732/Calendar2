...
<head>
    ...
    <link rel="stylesheet" href="/template/assets/css/admin_complete.css">
</head>
<body>
<div class="container">
        ...
        <?php foreach($tasks as $task): ?>
            <li data-date="<?=$task['deadline_day']?>" data-time="<?=$task['deadline_time']?>" class="today">
                <div class="task-content">
                    <div class="task-info">
                        <h3><?php if(isset($task['id']) && !empty($task['id'])){ ?><a href="/show/complete/task/<?=$task['id']?>"><?php } ?><?=$task['title']?></a></h3>
    ...
    <div style="display: flex;justify-content: space-around;">
        <a href="/show/project/<?=$project
        ?>" class="view-tasks-link">Назад к не Выполненым Задачам</a>
